...
public interface NearExpiryDao extends JpaRepository<MktDataDto, Integer> { 
	...
	@Query(value="CALL productwisenearexp(:depo_code,:login_id,:div_code,:code,:utype,:rep_type,:optn);", nativeQuery=true)
	List<NearExpiry> getNearExpiryReport(@Param("depo_code") int depo_code,
		   ...
		   @Param("rep_type") int rep_type,@Param("optn") int optn);
	

	
	
	
	@Query(value="CALL near_expiry_report_branchwise(:depo_code,:login_id,:div_code,:rep_type,:utype,:optn);", nativeQuery=true)
	List<NearExpiry> getNearExpiryReportBranchwise(@Param("depo_code") int depo_code,
			@Param("login_id") int login_id,
			@Param("div_code") int div_code,
			@Param("rep_type") int rep_type,
			@Param("utype") int utype,@Param("optn") int optn);
