...
public interface StkRepo2Dao extends JpaRepository<MktDataDto, Integer> {
	...
	@Query(value="CALL aristo_web.web_stockiest_repo2_credit(:myear,:div_code,:depo_code,:smon,:emon,:crn_type,:code,:login_id);", nativeQuery=true)
	List<StkRepo2> getStockiestRepo2Credit(@Param("myear") int myear,@Param("div_code") int div_code,@Param("depo_code") int depo_code,
			@Param("smon") int smon,@Param("emon") int emon,
			@Param("crn_type") int crn_type,@Param("code") String code,@Param("login_id") int login_id);
	
	@Query(value="CALL aristo_web.web_stockiest_repo2_credit_group(:myear,:div_code,:depo_code,:smon,:emon,:crn_type,:code,:login_id);", nativeQuery=true)
