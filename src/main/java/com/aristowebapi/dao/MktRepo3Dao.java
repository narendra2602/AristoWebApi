...
public interface MktRepo3Dao extends JpaRepository<MktDataDto, Integer> {
	...
	@Query(value="CALL web_report_target_trend(:myear,:div_code,:depo_code,:smon,:emon,:utype,:login_id,:rep_type,:hq_code);", nativeQuery=true)
	List<MktRepo3> getWebReportTargetTrend(@Param("myear") int myear,@Param("div_code") int div_code,@Param("depo_code") int depo_code,
			@Param("smon") int smon,@Param("emon") int emon,@Param("utype") int utype,@Param("login_id") int login_id,
			@Param("rep_type") int rep_type,@Param("hq_code") int hq_code);
