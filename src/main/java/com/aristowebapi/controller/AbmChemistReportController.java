...
//	    @PostMapping("/finalize")
	    public ResponseEntity<?> regenerateFinalReport( @PathVariable int monthCode,@PathVariable int  divCode,@PathVariable long  reportId,HttpServletRequest req){
	        try {

                  
	        	String response = service.regenerateAllFinalReports(monthCode,divCode,reportId);
	            return ResponseEntity.ok(response);
